...
function createMessagesRouter({
  ...
  router.get("/messages", (req, res) => {
    ...
    const rows = dbAll(
      db,
      `
        SELECT m.id, m.room_id, m.iv, m.ciphertext, m.created_at, u.username, u.avatar_mime, u.avatar_data
        FROM messages m
    ...
    const images = dbAll(
      db,
      `
        SELECT i.id, i.room_id, i.iv, i.ciphertext, i.created_at, i.mime, u.username, u.avatar_mime, u.avatar_data
        FROM images i
